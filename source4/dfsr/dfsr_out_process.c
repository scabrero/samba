...
static void dfsrsrv_process_next(struct tevent_req *subreq)
{
	...
	if (state->entry->update->present &&
		...
		DEBUG(8, ("dfsrsrv: Downloading full update {%s}-%lu (%s)\n",
			  GUID_buf_string(&state->entry->update->gsvn_db_guid,
					  &txtguid1),
			  state->entry->update->gsvn_version,
			  state->entry->update->name));
...
static void dfsrsrv_process_install_done(struct tevent_req *subreq)
{
	...
	DEBUG(6, ("dfsrsrv: Installed {%s}-%lu (%s): %s\n",
		  GUID_buf_string(&state->entry->update->gsvn_db_guid,
				  &txtguid1),
		  state->entry->update->gsvn_version,
	...
	if (tevent_req_nterror(req, status)) {
		DEBUG(0, ("dfsrsrv: Failed to delete staging file '%s': %s\n",
...
out:
	if (superseded) {
		...
		superseding_txt = talloc_asprintf(mem_ctx, "{%s-v%lu}",
				...
				stored->gsvn_version);
		DEBUG(6, ("dfsrsrv: Update %s superseded by %s\n",
	...
	if (entry == NULL) {
		...
		if (vv_queue->pending_updates == NULL) {
			...
			status = dfsrsrv_update_stored_vv(service,
					...
					vv_queue->vv_count);
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG(0, ("dfsrsrv: Failed to update stored"
					  " version vector for replica set "
					  "{%s} on replication group {%s}: "
					  "%s\n",
