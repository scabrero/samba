		...
		uint32_t vv_count)
{
	...
	DEBUG(5, ("dfsrsrv: Requesting updates [type %d] for content set "
...
static void dfsrsrv_session_request_updates_next(struct tevent_req *subreq)
{
	...
	if (tevent_req_nterror(req, status)) {
		DEBUG(0, ("dfsrsrv: Failed to request updates: %s\n",
			  nt_errstr(status)));
		return;
	}
	...
	for (i = 0; i < state->num_updates; i++) {
		...
		DEBUG(6, ("dfsrsrv: Queued update {%s}-v%lu (%s) "
...
filter_and_request_more:
	...
	status = dfsrsrv_lexicofilter_vector(state,
			...
			&new_vv, &new_vv_count);
	if (tevent_req_nterror(req, status)) {
		DEBUG(0, ("dfsrsrv: Failed to filter version vectors: "
			  "%s\n", nt_errstr(status)));
		return;
	}
	TALLOC_FREE(state->request_vv_delta);
...
request_more:
	...
	status = dfsrsrv_update_known_vv(state->set, state->queue->vv,
			state->queue->vv_count);
	if (tevent_req_nterror(req, status)) {
		DEBUG(0, ("dfsrsrv: Failed to update known version "
			  "vectors: %s\n", nt_errstr(status)));
		return;
	}
	...
	DEBUG(6, ("dfsrsrv: Queued version chain vector (Content set {%s}, "
		...
		struct dfsrsrv_session *session)
{
	...
	DEBUG(1, ("dfsrsrv: Establish DFS-R session for content set {%s} "
		  "over DFS-R connection {%s}\n",
...
static void dfsrsrv_establish_session_done(struct tevent_req *subreq)
{
	...
	if (tevent_req_nterror(req, status)) {
		DEBUG(0, ("dfsrsrv: Failed to establish DFS-R session for "
	...
	if (!W_ERROR_IS_OK(result)) {
		...
		if (W_ERROR_EQUAL(result,
			WERR_FRS_ERROR_CONNECTION_INVALID)) {
	...
	DEBUG(5, ("dfsrsrv: Registering version chain vector generation %lu "
...
static void dfsrsrv_session_request_vv_done(struct tevent_req *subreq)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(0, ("dfsrsrv: Failed to request version vectors for "
			  "content set {%s} on connection {%s}: %s\n",
			  GUID_buf_string(&session->set->guid, &txtguid1),
			  GUID_buf_string(&session->conn->guid, &txtguid2),
			  nt_errstr(status)));
...
static void dfsrsrv_session_done(struct tevent_req *req)
{
	...
	DEBUG(0, ("dfsrsrv: Content set {%s} session on connection {%s} "
		  "terminated\n",
		  GUID_buf_string(&session->set->guid, &txtguid1),
		  GUID_buf_string(&session->conn->guid, &txtguid2)));
		...
		struct dfsrsrv_connection *conn)
{
	...
	DEBUG(1, ("dfsrsrv: Establish DFS-R connection {%s} to '%s'\n",
...
static void dfsrsrv_establish_connection_done(struct tevent_req *subreq)
{
	...
	if (tevent_req_nterror(req, status)) {
		DEBUG(0, ("dfsrsrv: Failed to establish DFS-R connection "
			  "{%s}: %s\n", GUID_buf_string(&state->conn->guid,
							&txtguid),
			  nt_errstr(status)));
...
static void dfsrsrv_connection_asyncpoll_done(struct tevent_req *subreq)
{
	...
	if (session == NULL || session_state == NULL) {
		DEBUG(0, ("dfsrsrv: Got an AsyncPoll response (seq num %d) "
			  "but no matching session found\n",
	...
	if (session_state->request_type !=
		...
		DEBUG(1, ("dfsrsrv: Request type %d not supported\n",
		...
		struct dfsrsrv_session_state *session_state)
{
	...
	DEBUG(5, ("dfsrsrv: Version chain vector changed, generation %lu "
		  "available for content set {%s} on connection {%s}\n",
	...
	DEBUG(5, ("dfsrsrv: Requesting version chain vector generation %lu "
		  "for content set {%s} on connection {%s}\n",
		  session_state->vv_generation,
		  GUID_buf_string(&session_state->session->set->guid,
				  &txtguid1),
		  GUID_buf_string(&session_state->session->conn->guid,
		...
		struct frstrans_AsyncResponseContext *response)
{
	...
	/* [MS-FRS2] 3.3.4.5 The server has sent its version chain
	 * vector */
	DEBUG(5, ("dfsrsrv: Received version chain vector generation "
	...
	if (tevent_req_nterror(session_state->session->req, status)) {
		DEBUG(0, ("dfsrsrv: Failed to compute version vector delta for"
			  " content set {%s} on connection {%s}: %s\n",
			  GUID_buf_string(&session_state->session->set->guid,
					  &txtguid1),
			  GUID_buf_string(&session_state->session->conn->guid,
	...
	DEBUG(5, ("dfsrsrv: Computed version vector delta for content set {%s}"
		  " on connection {%s}:\n",
		  GUID_buf_string(&session_state->session->set->guid,
				  &txtguid1),
		  GUID_buf_string(&session_state->session->conn->guid,
	...
	if (session_state->vv_delta_count == 0) {
		...
		DEBUG(5, ("dfsrsrv: Registering version chain vector "
...
static void dfsrsrv_connection_done(struct tevent_req *req)
{
	...
	DEBUG(0, ("dfsrsrv: DFS-R Connection {%s} to '%s' terminated\n",
				 ...
				 struct dfsrsrv_connection *conn)
{
	...
	if (!conn->enabled) {
		DEBUG(3, ("dfsrsrv: Connection {%s} on replication group {%s}"
			  " is disabled\n",
	...
	for (session = conn->sessions; session; session = session->next) {
		if (!session->set->enabled) {
			DEBUG(3, ("dfsrsrv: Content set {%s} on replication "
				  "group {%s} is disabled\n",
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG(0, ("dfsrsrv: Failed to run content set {%s} "
				  "session on connection {%s}: %s\n",
				  GUID_buf_string(&session->set->guid,
						  &txtguid1),
...
NTSTATUS dfsrsrv_run_subscriptions(struct dfsrsrv_service *service)
{
	...
	DEBUG(6, ("dfsrsrv: Running subscriptions\n"));
	...
	for (group = service->subscriptions; group; group = group->next) {
		for (conn = group->connections; conn; conn = conn->next) {
			...
			if (!NT_STATUS_IS_OK(status)) {
				struct GUID_txt_buf txtguid1, txtguid2;
				DEBUG(0, ("dfsrsrv: Failed to run replication "
					  "group {%s} connection {%s}: %s\n",
