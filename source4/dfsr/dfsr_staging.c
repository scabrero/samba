				      ...
				      bool append)
{
	...
	if (fd == -1) {
		DEBUG(0, ("dfsrsrv: Unable to create staging file %s: %s\n",
	...
	if (buffer_size != 0) {
		...
		if (w_size != buffer_size) {
			DEBUG(0, ("dfsrsrv: Unable to write %u bytes on "
				  "staging file %s: %s\n",
				  buffer_size, staging_file, strerror(errno)));
			close(fd);
			talloc_free(tmp_ctx);
	...
	ret = close(fd);
	if (ret != 0) {
		DEBUG(0, ("dfsrsrv: Unable to close %s: %s\n",
			  staging_file, strerror(errno)));
		talloc_free(tmp_ctx);
		return NT_STATUS_FILE_INVALID;
	}
