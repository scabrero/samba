				    ...
				    struct ldb_dn *computer_account_dn)
{
	...
	sysvol_join = lpcfg_parm_bool(service->task->lp_ctx, NULL, "dfsrsrv",
				      "sysvol_join", false);
	if (!sysvol_join) {
		DEBUG(2, ("dfsrsrv: Skip adding ourselves as a member of "
	...
	if (ret != LDB_SUCCESS) {
		DEBUG(0, ("dfsrsrv: Failed to start ldb transaction: %s\n",
	...
	if (ret != LDB_SUCCESS) {
		DEBUG(0, ("dfsrsrv: Failed to add '%s' to sysvol replication "
			  "group '%s': %s\n",
			  ldb_dn_get_linearized(computer_account_dn),
			  ldb_dn_get_linearized(sysvol_group_dn),
			  ldb_errstring(service->samdb)));
	...
	DEBUG(2, ("dfsrsrv: Subscribing ourselves to sysvol replication "
		  "group\n"));
	...
	ret = ldb_add(service->samdb, msg);
	if (ret != LDB_SUCCESS) {
		DEBUG(0, ("dfsrsrv: Failed to add sysvol replication group "
			  "subscription: %s\n",ldb_errstring(service->samdb)));
		status = dsdb_ldb_err_to_ntstatus(ret);
		goto fail;
	}
	...
	DEBUG(2, ("dfsrsrv: Subscribing ourselves to sysvol content set\n"));
	...
	ret = ldb_msg_add_string(msg, "msDFSR-StagingPath",
			staging_path);
	if (ret != LDB_SUCCESS) {
		DEBUG(0, ("dfsrsrv: Failed to add message attribute: %s\n",
			  ldb_errstring(service->samdb)));
	...
	ret = ldb_add(service->samdb, msg);
	if (ret != LDB_SUCCESS) {
		DEBUG(0, ("dfsrsrv: Failed to add content set subscription: "
			  "%s\n", ldb_errstring(service->samdb)));
		status = dsdb_ldb_err_to_ntstatus(ret);
		goto fail;
	}
	...
	if (ret != LDB_SUCCESS) {
		DEBUG(0, ("dfsrsrv: Failed to commit samdb transaction: %s",
			  ldb_errstring(service->samdb)));
		status = dsdb_ldb_err_to_ntstatus(ret);
		return status;
	}
...
NTSTATUS dfsrsrv_sysvol_subscription_check(struct dfsrsrv_service *service)
{
	...
	if (ret != LDB_SUCCESS) {
		DEBUG(0, ("dfsrsrv: Failed to search sysvol replication "
			  "group: '%s'", ldb_errstring(service->samdb)));
		status = dsdb_ldb_err_to_ntstatus(ret);
		goto out;
	}
	if (res->count != 1) {
		DEBUG(0, ("dfsrsrv: Failed to search sysvol replication "
			  "group, expected one entry but %d found\n",
			  res->count));
		status = NT_STATUS_NOT_FOUND;
		goto out;
	}
	...
	if (ret != LDB_SUCCESS) {
		DEBUG(0, ("dfsrsrv: Failed to search members of sysvol "
			  "replication group: %s\n",
			  ldb_errstring(service->samdb)));
		status = dsdb_ldb_err_to_ntstatus(ret);
		goto out;
	...
	if (res->count == 0) {
		...
		if (ret != LDB_SUCCESS) {
			DEBUG(0, ("dfsrsrv: Failed to search sysvol content "
				  "set: %s\n", ldb_errstring(service->samdb)));
			status = dsdb_ldb_err_to_ntstatus(ret);
			goto out;
		}
		 ...
		 * set */
		if (res2->count != 1) {
			DEBUG(0, ("dfsrsrv: Content set not found in sysvol "
				  "replication group '%s'\n",
				  sysvol_group_name));
			status = NT_STATUS_NOT_FOUND;
			goto out;
		...
		struct ldb_dn *member_dn)
{
	...
	if (res->count != 1) {
		DEBUG(0, ("dfsrsrv: Failed to search replication group {%s} "
			  "member '%s': Expected one entry but %d found\n",
			  GUID_buf_string(&group->guid, &txtguid1),
	...
	if (ret != LDB_SUCCESS) {
		DEBUG(0, ("dfsrsrv: Failed to search nTDSConnection: %s\n",
			  ldb_errstring(service->samdb)));
		return dsdb_ldb_err_to_ntstatus(ret);
	}
	...
	for (i = 0; i < res->count; i++) {
		...
		if (ret != LDB_SUCCESS) {
			DEBUG(0, ("dfsrsrv: Failed to search dnsHostName: "
				  "%s\n", ldb_errstring(service->samdb)));
			continue;
		}
		...
		if (res2->count != 1) {
			DEBUG(0, ("dfsrsrv: Failed to search dnsHostName: "
				  "Expected one entry buf %d found.\n",
				   res2->count));
			TALLOC_FREE(res2);
			continue;
		}
