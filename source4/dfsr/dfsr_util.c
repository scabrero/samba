		...
		uint32_t vv_count)
{
	...
	DEBUG(5, ("dfsrsrv: Content set {%s} known version vectors "
		  "updated to:\n", GUID_buf_string(&set->guid, &txtguid1)));
	for (i = 0; i < set->known_vv_count; i++) {
		DEBUG(5, ("\t{%s} - [%lu, %lu]\n", GUID_buf_string(
				&set->known_vv[i].db_guid, &txtguid2),
			  set->known_vv[i].low,
			  set->known_vv[i].high));
	}
				  ...
				  uint32_t vv_count)
{
	...
	if (NT_STATUS_EQUAL(NT_STATUS_NOT_FOUND, status)) {
		...
		DEBUG(0, ("dfsrsrv: Error fetching stored version vectors for "
	...
	DEBUG(6, ("dfsrsrv: Updating stored content set {%s} known version "
		  "vectors to:\n", GUID_buf_string(&set->guid, &txtguid1)));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(0, ("dfsrsrv: Error storing version vectors for replica "
				     ...
				     uint32_t *out_count)
{
	...
	DEBUG(7, ("dfsrsrv: Filtering version vectors by {%s}-%lu\n",
