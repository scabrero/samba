		...
		struct smb_filename **fname)
{
	...
	if (update->uid_version == 1) {
		/* The root folder */
		*fname = synthetic_smb_fname(mem_ctx, ".", NULL, NULL, 0);
		if (*fname == NULL) {
	...
	*fname = synthetic_smb_fname(mem_ctx,
		talloc_asprintf(mem_ctx, "%s/%s", (*fname)->base_name,
			update->name), NULL, NULL, 0);
	if (*fname == NULL) {
		status = NT_STATUS_NO_MEMORY;
		goto out;
	}
		...
		off_t *offset)
{
	...
	DEBUG(8, ("dfsrmeet: Unmarshalling metadata stream\n"));
	...
	*offset += nread;
	...
	data_blob_free(&in);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG(0, ("dfsrmeet: Failed to pull metadata stream: %s\n",
			  ndr_errstr(ndr_err)));
		status = ndr_map_error2ntstatus(ndr_err);
		goto out;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(0, ("dfsrmeet: Failed to open or create file '%s': %s\n",
		    ...
		    strcasecmp_m(rec->update->name, update->name) != 0))
	{
		...
		DEBUG(8, ("dfsrmeet: Renaming '%s' to '%s'\n",
			  smb_fname->base_name, new_smb_fname->base_name));

		status = rename_internals_fsp(conn, *fsp, new_smb_fname,
					  0, false);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG(0, ("dfsrmeet: Failed to rename '%s' "
				  "to '%s': %s\n",
				  smb_fname->base_name,
...
out:
	...
	if (fsp != NULL) {
		...
		if (!NT_STATUS_IS_OK(status2)) {
			DEBUG(0, ("dfsrmeet: Failed to close file: %s\n",
				  nt_errstr(status2)));
		}
	}
